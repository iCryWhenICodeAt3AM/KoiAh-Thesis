...
function recommendPondDimensions(rule) {
    const depth = 3;
    const averageKoiLength = (document.getElementById("sizeOfKoiFish").value) ? document.getElementById("sizeOfKoiFish").value : null;
    ...
    if (rule === 1) {
        ...
        if (averageKoiLength === null || numKoi === null) {
            console.error("Both average koi length and number of koi must be provided for Rule 1.");
            return;
        }
        volumeGallons = averageKoiLength * 10 * numKoi;
        formulaCont.innerText = `(${averageKoiLength} * 10 * ${numKoi})`;
    ...
    console.log(`Inputs: Rule = ${rule}, Ave Size = ${averageKoiLength}, number = ${numKoi}`);
